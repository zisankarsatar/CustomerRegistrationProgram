<?php     
    $cari_adlar=$db->query("SELECT * FROM cari WHERE durum IN (0)  ORDER BY ad ASC ")->fetchAll(PDO::FETCH_ASSOC);
    ...
    if(isset($_POST["submit"])){
      ...
      if(!$cari_id){
          echo "Lütfen cari id belirleyiniz.";
      }elseif(!$tarih){
          ...
          $ekle=$sorgu->execute([
              $cari_id,$tarih,$gorusme_tipi,$lokasyon,$aciklama
          ]);
...
<body>
<div id="icerik">
<button class="btn btn-info" onClick="location.href='index.php?sayfa=carisizEkle'">Carisiz Görüşme Ekle</button><hr>
<form action=""  method="post" ALIGN="center">

    CARİ FİRMA ADI:<br>
        <select name="cari_id">
            <?php foreach ($cari_adlar as $cari_ad): ?>
                <option value="<?php echo $cari_ad['id']?>"> <?php echo $cari_ad['ad']?></option>
            <?php endforeach; ?>
        </select><br><br>
    Tarih:<br>
